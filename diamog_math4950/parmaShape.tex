...
Instead ParMA uses the mesh adjacencies and partition model data.
Since ParMA has all the mesh and model data including topological and geometric
coordinates, it can be used for both (hyper)graph and geometric based 
procedures which use only a portion of the data held by ParMA.
...
We plan to utilize ParMA to target the overall shape of a part in order to reduce the number of neighbors each part has and thus reduce communication times.  
